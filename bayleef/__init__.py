...
def loop_params(config, params, not_ok, errors, file=False):
    """loops over keys in 'params' in the config dict."""
    for param in params:
        ok, msg = check_file_or_path(config[param], file)
        if not ok:
            not_ok = True
            errors.append(f"\t'{param}' {msg}.")
    return not_ok, errors    
...
def config_check(config, config_path):
    ...
    # I am assuming the config file location and keys do not need to be checked. 
    # The copyfile command below should throw an exception so we don't need to here.
    not_ok, errors = loop_params(config, ['data', 'davinci_bin'], not_ok, errors)
    not_ok, errors = loop_params(config['themis'], ['map_file'], not_ok, errors, file=True)
    warn, warnings = loop_params(config['themis'], ['controlled_kernels'], False, warnings)
    if not_ok:
        if warn: 
            errors += warnings
        raise USGSError('\n'.join(errors))
